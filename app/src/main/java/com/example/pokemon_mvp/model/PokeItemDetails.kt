...
import com.example.pokemon_mvp.response.Types
...
fun PokemonDetails.toDomain() : PokeItemDetails {
    val id = "N° ${id.toString().padStart(3,'0')}"
    val name = replaceFirstChar(name)
    val img = sprites.other.officialArtwork.img
    val hp = stats[0].statValue
    val attack = stats[1].statValue
    val defense = stats[2].statValue
    val specialAttack = stats[3].statValue
    val specialDefense = stats[4].statValue
    val speed = stats[5].statValue
